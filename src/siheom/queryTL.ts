...
export type TLocator = {
    ...
    fill(text: string, options?: Parameters<typeof userEvent.type>[2]): Promise<void>;
    clear(): Promise<void>;
...
const ARIACompositeWidgetRole = ['combobox', 'grid', 'listbox', 'menu', 'menubar', 'radiogroup', 'tablist', 'tree', 'treegrid'] as const;
...
const ARIALiveRegionRole = ['alert', 'log', 'marquee', 'status', 'timer'] as const;

const ARIAWindowRole = ['alertdialog', 'dialog'] as const;

const ARIALandmarkRole = ['banner', 'complementary', 'contentinfo', 'form', 'main', 'navigation', 'region', 'search'] as const;
export const roles = [
